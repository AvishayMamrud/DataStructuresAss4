...
//        Node<T> node = this.getNode(value);
    ...
    public boolean insert2pass(T value) {
        Node<T> node = root;
        Node<T> upperMost = root;
        while (node != null) {
            // System.out.println(node.toString());
            // System.out.println(maxKeySize+ "max");
		...
		return add(upperMost, value);
    }
    
    /**
     * {@inheritDoc}
     */
    public boolean add(Node<T> node, T value) {
        if (node == null) {
            ...
            while (node != null) {
               ...
               // System.out.println(maxKeySize+ "max");
                if(node.keysSize==maxKeySize){
                    split(node);
                    int index = getIndexOfNextValue(node.parent, node.getKey(0));
                    node = node.parent.getKey(index).compareTo(value)<0 ? node.parent.getChild(index + 1) : node.parent.getChild(index);
                }
                if (node.numberOfChildren() == 0) {
                    node.addKey(value);
                    break;
    ...
    private void split(Node<T> nodeToSplit) {
        ...
        if (node.parent == null) {
         ...
         //   System.out.println("remove child"+ node.toString());
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
        	...
        	if(lesser.numberOfKeys()==minKeySize && greater.numberOfKeys()==minKeySize) {
        		Node<T> merged = this.mergeWithSons(node, value);
        		remove(value, merged); 
        	}else if(lesser.numberOfKeys()==minKeySize) {
        		removed = node.removeKey(value);
        		Node<T> smallest = this.getSmallestNode(greater);//
	            T replaceValue = smallest.getKey(0);
	            node.addKey(replaceValue);
	            remove(replaceValue, greater);        		
        	}else {
        		removed = node.removeKey(value);
        		Node<T> greatest = this.getGreatestNode(lesser);//
	            T replaceValue = greatest.getKey(greatest.numberOfKeys() - 1);
	            node.addKey(replaceValue);
	            remove(replaceValue, lesser);
        	}
            
            //this.removeGreatestValue(greatest);
    ...
    private boolean combined(Node<T> node) {
        ...
        if (leftNeighbor != null && leftNeighborSize > minKeySize) {
            ...
            if (rightNeighbor != null && rightNeighborSize > minKeySize) {
            	...
            	int prev = getIndexOfNextValue(parent, leftNeighbor.getKey(leftNeighbor.numberOfKeys()-1));
            	node = mergeWithSons(parent, parent.getKey(prev));
...
//               } else
    ...
    private Node<T> mergeWithSons(Node<T> nodeToMerge, T value) {
        
    	int index = nodeToMerge.indexOf(value);
        int indexOfLeftNeighbor = index;
    	int indexOfRightNeighbor = index + 1;
    	Node<T> parent = nodeToMerge;
    	Node<T> node = nodeToMerge.getChild(indexOfRightNeighbor);    	
